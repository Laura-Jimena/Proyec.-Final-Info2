...
class ECGSignal:
    ...
    def graficar_derivacion(self,t,derivacion):
      ...
      if self.MostrarDatos() is not None:
        ...
        fig=Figure(figsize=(16, 4))  # Ajustar el ancho
        ...
        ax.plot(tiempo, self.MostrarDatos()[fila,:], linewidth=0.5)
        ax.set_title(f'ECG - Derivacion {derivacion}')
        ax.plot(self.MostrarIndicesPicos(), data[self.MostrarIndicesPicos()], 'ro', label="Picos R") #señala los picos R en la grafica
        ax.set_xlabel('Tiempo (ms)')
        ax.set_ylabel('Amplitud (mV)')

        # Establecer límites de los ejes

        ax.set_xlim(0, tiempo[t])  # Limitar el eje X al rango de tiempo
        ax.set_ylim(signal_min, signal_max)  # Limitar el eje Y según los valores min/max de la señal
        ax.grid()
        return fig
      else:
    ...
    def imprimir_estadisticas(self,derivacion):
       ...
       Return: Texto con estadísticas calculadas.
       """
      # Definimos un diccionario para almacenar las métricas por derivación
       ...
       resultado=[]
       resultado.append(f"Estadísticas calculadas para la derivación {derivacion}")
       resultado.append(" ")

       for key, valor in estadisticas[derivacion].items():
          resultado.append(f"{key}: {valor if valor is not None else 'No calculado'}")
    ...
    def graficar_todas_las_derivaciones(self, t):
      ...
      if self.MostrarDatos() is not None:
        ...
        ax.plot(tiempo, data_II + desplazamiento_II, label="Derivación II", linewidth=0.5)
        ax.plot(tiempo, data_V2 + desplazamiento_V2, label="Derivación V2", linewidth=0.5)
        ax.plot(tiempo, data_V5 + desplazamiento_V5, label="Derivación V5", linewidth=0.5)
        ax.set_title('ECG - Derivaciones II, V2 y V5')
        ax.set_xlabel('Tiempo (ms)')
        ax.set_ylabel('Amplitud (mV)')
