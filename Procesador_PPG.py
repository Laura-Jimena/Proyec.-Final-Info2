...
class PPG:
    ...
    def Asignarseñal(self, señal):
        ...
        if type(señal)==pd.DataFrame:
            self.__señalppgdf = señal
        else:
        ...
        try:
            ...
            if isinstance(archivo, pd.DataFrame):
                if fila < len(archivo):
                    ...
                    ax.plot(señal)
                    ax.set_title(f"Señal PPG - Paciente {fila}")
                    ax.set_xlabel("Tiempo (o índice de columna)")
                    ax.set_ylabel("Amplitud de la señal PPG")
    ...
    def analizar_frecuencia_ppg_por_fila(self,df, fila, fs, umbral_baja=3, umbral_alta=5, ventana_tamaño=1024, solapamiento=512):
        """
        ...
        signal =pd.to_numeric(df.iloc[fila],errors="coerce").values
        ...
        for start in range(0, n_muestras - ventana_tamaño, ventana_tamaño - solapamiento):
            ...
            segmento=np.asarray(segmento, dtype=float)
            segmento = np.nan_to_num(segmento)
        ...
        ax.plot(signal, label="Señal PPG")
        for (start, end) in rangos_bajos:
            ax.axvspan(start, end, color='green', alpha=0.3, label='Frecuencia Baja')
        for (start, end) in rangos_altos:
            ax.axvspan(start, end, color='red', alpha=0.3, label='Frecuencia Alta')

        ax.legend()
        ax.set_xlabel('Tiempo (índice de muestra)')
        ax.set_ylabel('Amplitud')
        ax.set_title('Análisis de frecuencias en la señal PPG')
    ...
    def indicenormalidad(self, archivo, paciente):
        ...
        try:
            ...
            if etiqueta == 'MI':
                mi=f"El paciente {paciente} presenta anormalidad cardíaca."
                return mi
            elif etiqueta == 'N':
                n=f"El paciente {paciente} no presenta anormalidad cardíaca."
                return n
            else:
                desc=(f"El paciente {paciente} tiene una etiqueta desconocida: {etiqueta}.")
                return desc
        except Exception as e:
            print(f"Error al procesar el archivo: {e}")
